...
const fetchUser = createAsyncThunk("user/fetchUser", async (token) => {
  try {
    const response = await axios.post(
      `${process.env.EXPO_PUBLIC_SERVER_IP}/api/users/token`,
      {
        token: token,
      }
    );
...
export const updateUserData = createAsyncThunk(
  'user/update',
  async (data) => {
      // Adjusted to accept an object with `data` and `id` properties
      try {
          console.log("Redux?", data);
          await axios.patch(
              `${process.env.EXPO_PUBLIC_SERVER_IP}/api/users/updateUser/${data.id}/`,
              data.data
          );
          await AsyncStorage.setItem('userId', JSON.stringify(data.id));
          // console.log("AsyncStorageId is : ", await AsyncStorage.getItem('userId'));
          // Assuming you want to return the updated user data or a success message
          // return { success: true }; // Example return statement
      } catch (err) {
          console.log("error coming from updateUserData", JSON.stringify(err));
          throw err;
      }
  }
);
...
export const autoLogin = createAsyncThunk(
  ...
  async (test, thunkAPI) => {
    try {
      ...
      const response = await axios.post(
        `${process.env.EXPO_PUBLIC_SERVER_IP}/api/users/reniewToken`,
        { token: token }
      );
  ...
  async (inputForm) => {
    try {
      const task = await axios.post(
        `${process.env.EXPO_PUBLIC_SERVER_IP}/api/report/create`,
        inputForm
      );
      return task.data;
    } catch (error) {
...
//        const response = await axios.post(
//          `${process.env.EXPO_PUBLIC_SERVER_IP}/api/users/emailLogin`,
//          {
//            email: data.userDetails.email,
//            password: data.userDetails.password,
//          }
...
export const getOneById = createAsyncThunk("user/getOneById", async (id) => {
  try {
    const res = await axios.get(
      `${process.env.EXPO_PUBLIC_SERVER_IP}/api/users/getOne/${id}`
    );
