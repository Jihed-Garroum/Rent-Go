  ...
  async (params) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.EXPO_PUBLIC_SERVER_IP}/api/request/create/${params.id}`,
        params.body
      );
      const requestId = response.data.id;
      await axios.post(
        `${process.env.EXPO_PUBLIC_SERVER_IP}/api/media/add/request/${requestId}`,
        params.media
      );
...
export const getAgencyData = createAsyncThunk("agencies/getAll", async () => {
  try {
    const res = await axios.get(
      `${process.env.EXPO_PUBLIC_SERVER_IP}/api/agency/findAll`
    );
    return res.data;
  } catch (er) {
...
export const getOne = createAsyncThunk("agencies/getOne", async (id) => {
  try {
    ...
    const res = await axios.get(
      `${process.env.EXPO_PUBLIC_SERVER_IP}/api/agency/getOne/${id}`
    );
